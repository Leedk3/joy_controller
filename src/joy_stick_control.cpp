...
class JOY_CONTROL
{
    ...
    private:        
        ros::NodeHandle nh;
        ros::Subscriber SubJoyCommand, SubAutoCommand, SubVisualCommand;
        ros::Publisher PubVehCommand, pubCameraMotorInit;

        ackermann_msgs::AckermannDriveStamped VehCommand;
        ackermann_msgs::AckermannDriveStamped m_AutoCommand, m_VisualCommand;

        bool AutonmousMode, VisualMode, bManualMode;
...
JOY_CONTROL::JOY_CONTROL(ros::NodeHandle& n) : nh(n), maximum_steering_angle(20), maximum_speed(5.0),
                                               AutonmousMode(false), init_camera_motor(false), bManualMode(false)
{
    // Subscribers
